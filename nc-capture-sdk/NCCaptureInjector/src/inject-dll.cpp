...
int inject_dll_enc(HANDLE process, const wchar_t *dll,
	...
	const char *load_library_w_obf, uint64_t obf5)
{
	...
	create_remote_thread = (create_remote_thread_t)get_encrypted_func(kernel32, create_remote_thread_obf, obf1);
	write_process_memory = (write_process_memory_t)get_encrypted_func(kernel32, write_process_memory_obf, obf2);
	virtual_alloc_ex = (virtual_alloc_ex_t)get_encrypted_func(kernel32, virtual_alloc_ex_obf, obf3);
	virtual_free_ex = (virtual_free_ex_t)get_encrypted_func(kernel32, virtual_free_ex_obf, obf4);
	load_library_w = (FARPROC)get_encrypted_func(kernel32, load_library_w_obf, obf5);
	...
	if (WaitForSingleObject(thread, 4000) == WAIT_OBJECT_0) {
		...
		ret = (code != 0) ? 0 : eIE_INJECT_FAILED;

		SetLastError(0);
	}

fail:
	if (ret == eIE_UNLIKELY_FAIL) {
...
int inject_dll_safe_enc(DWORD thread_id, const wchar_t *dll,
	const char *set_windows_hook_ex_obf, uint64_t obf1)
{
	...
	set_windows_hook_ex = (set_windows_hook_ex_t)get_encrypted_func(user32, set_windows_hook_ex_obf, obf1);

	hook = set_windows_hook_ex(WH_GETMESSAGE, (HOOKPROC)proc, lib, thread_id);
	if (!hook) {
		return GetLastError();
	}
