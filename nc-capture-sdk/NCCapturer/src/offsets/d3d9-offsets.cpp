...
static inline bool d3d9_init(d3d9_info &info)
{
	...
	if (!info.hwnd) {
		MessageBoxA(NULL, "failed", "d3d9_init_createwindow", MB_OK);
		return false;
	}

	info.module = LoadLibraryA("d3d9.dll");
	if (!info.module) {
		MessageBoxA(NULL, "failed", "d3d9_init_loadlib", MB_OK);
		return false;
	}
	...
	create = (d3d9createex_t)GetProcAddress(info.module,
						"Direct3DCreate9Ex");
	if (!create) {
		MessageBoxA(NULL, "failed", "d3d9_init_getprocadress", MB_OK);
		return false;
	}

	hr = create(D3D_SDK_VERSION, &info.d3d9ex);
	if (FAILED(hr)) {
		MessageBoxA(NULL, "failed", "d3d9_init_created3d", MB_OK);
		return false;
	}
