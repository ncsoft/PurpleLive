...
static bool create_d3d11_stage_surface(ID3D11Texture2D **tex)
{
	...
	hlog("[NCCAPTURER::d3d11-capture.cpp!::create_d3d11_stage_surface] device(0x%x) CreateTexture2D", data.device);
			     ...
			     HANDLE *handle)
{
	...
	hlog("[NCCAPTURER::d3d11-capture.cpp!::create_d3d11_tex] device(0x%x) handle(0x%x), format(%d) flags(0x%x) misc_flags(0x%x)", data.device, handle, data.format, flags, misc_flags);
	...
	hlog("[NCCAPTURER::d3d11-capture.cpp!::create_d3d11_tex] device(0x%x) CreateTexture2D", data.device);
	hr = data.device->CreateTexture2D(&desc, nullptr, tex);
	if (FAILED(hr)) {
	...
	hlog("[NCCAPTURER::d3d11-capture.cpp!::create_d3d11_tex] CreateRenderTargetView");
	if (!!render_target) {
	...
	hlog("[NCCAPTURER::d3d11-capture.cpp!::create_d3d11_tex] GetSharedHandle");
	if (!!handle) {
	...
	hlog("[NCCAPTURER::d3d11-capture.cpp!::create_d3d11_tex] success");
...
{
	hlog("[NCCAPTURER::d3d11-capture.cpp!::d3d11_init_scaling] create_d3d11_tex()");
...
static bool d3d11_shmem_init_buffers(size_t idx)
{
	...
	hlog("[NCCAPTURER::d3d11-capture.cpp!::d3d11_shmem_init_buffers] create_d3d11_tex()");
...
{
	hlog("[NCCAPTURER::d3d11-capture.cpp!::d3d11_shtex_init] create_d3d11_tex()");
...
{
	hlog("[NCCAPTURER::d3d11-capture.cpp!::d3d11_init]");
...
void d3d11_capture(void *swap_ptr, void *backbuffer_ptr, bool)
{
	...
	if (capture_should_init()) {
		hlog("[NCCAPTURER::d3d11-capture.cpp!::d3d11_capture] d3d11_init(swap)");
		d3d11_init(swap);
	}
	if (capture_ready()) {
