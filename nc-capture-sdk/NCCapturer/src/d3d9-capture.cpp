...
static struct detour_hook present_ex;
static struct detour_hook present_swap;
static struct detour_hook reset;
static struct detour_hook reset_ex;
	...
	HWND override_window, CONST RGNDATA *dirty_region, DWORD flags)
{
	...
	present_ex_t call = (present_ex_t)present_ex.pOrigin;
	hr = call(device, src_rect, dst_rect, override_window, dirty_region, flags);
	...
	HWND override_window, CONST RGNDATA *dirty_region, DWORD flags)
{
	...
	present_swap_t call = (present_swap_t)present_swap.pOrigin;
	hr = call(swap, src_rect, dst_rect, override_window, dirty_region, flags);
					    ...
					    D3DPRESENT_PARAMETERS *params)
{
	...
	reset_t call = (reset_t)reset.pOrigin;
					       ...
					       D3DDISPLAYMODEEX *dmex)
{
	...
	reset_ex_t call = (reset_ex_t)reset_ex.pOrigin;
...
static void setup_reset_hooks(IDirect3DDevice9 *device)
{
	...
	detour_hook_start(&reset, (void *)vtable[16], (void *)hook_reset, "IDirect3DDevice9::Reset");

	hr = device->QueryInterface(__uuidof(IDirect3DDevice9Ex),
				    (void **)&d3d9ex);
	if (SUCCEEDED(hr)) {
		detour_hook_start(&reset_ex, (void *)vtable[132], (void *)hook_reset_ex, "IDirect3DDevice9Ex::ResetEx");
		d3d9ex->Release();
	}
...
bool hook_d3d9(void)
{
	...
	if (present_swap_addr) {
		detour_hook_start(&present_swap, present_swap_addr, (void *)hook_present_swap, "IDirect3DSwapChain9::Present");
	}
	if (present_ex_addr) {
		detour_hook_start(&present_ex, present_ex_addr, (void *)hook_present_ex, "IDirect3DSwapChain9::Present");
	}
	if (present_addr) {
		detour_hook_start(&present, present_addr, (void *)hook_present, "IDirect3DDevice9::Present");
	}
...
void unhook_d3d9(void)
{
	hlog("[NCCAPTURER::d3d9-capture.cpp!::unhook_d3d9]");

	detour_hook_stop(&present);
	detour_hook_stop(&present_ex);
	detour_hook_stop(&present_swap);
	detour_hook_stop(&reset);
	detour_hook_stop(&reset_ex);
}

bool hook_d3d9_reset_when_jmp_patched(void)
{
	hlog("[NCCAPTURER::d3d9-capture.cpp!::hook_d3d9_reset_when_jmp_patched]");

	bool hook_complete_1 = detour_hook_is_complete_when_jmp_patched(&present);
	bool hook_complete_2 = detour_hook_is_complete_when_jmp_patched(&present_ex);
	bool hook_complete_3 = detour_hook_is_complete_when_jmp_patched(&present_swap);
	bool hook_complete_4 = detour_hook_is_complete_when_jmp_patched(&reset);
	bool hook_complete_5 = detour_hook_is_complete_when_jmp_patched(&reset_ex);
	bool res = false;
	...
	for (; i<DETOUR_JUMP_PATCHED_MAX_RETRY_COUNT; i++)
	{
		...
		if (!hook_complete_2)
			hook_complete_2 = detour_hook_restart_when_jmp_patched(&present_ex);
		if (!hook_complete_3)
			hook_complete_3 = detour_hook_restart_when_jmp_patched(&present_swap);
		if (!hook_complete_4)
			hook_complete_4 = detour_hook_restart_when_jmp_patched(&reset);
		if (!hook_complete_5)
			hook_complete_5 = detour_hook_restart_when_jmp_patched(&reset_ex);

		if (hook_complete_1 && hook_complete_2 && hook_complete_3 && hook_complete_4 && hook_complete_5) {
			res = true;
			break;
		}
	}
	hlog("[NCCAPTURER::d3d9-capture.cpp!::hook_d3d9_reset_when_jmp_patched] loop(%d) res(%d)", i, res);
	return true;
}