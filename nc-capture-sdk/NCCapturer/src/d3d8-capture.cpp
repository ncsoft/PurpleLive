...
static void setup_reset_hooks(IDirect3DDevice8 *device)
{
	...
	detour_hook_start(&reset, (void *)vtable[14], (void *)hook_reset, "IDirect3DDevice8::Reset");
...
bool hook_d3d8(void)
{
	...
	detour_hook_start(&present, present_addr, (void *)hook_present, "IDirect3DDevice8::Present");

	hlog("Hooked D3D8");
	return true;
}

void unhook_d3d8(void)
{
	hlog("[NCCAPTURER::d3d8-capture.cpp!::unhook_d3d8]");

	detour_hook_stop(&present);
	detour_hook_stop(&reset);
}

bool hook_d3d8_reset_when_jmp_patched(void)
{
	hlog("[NCCAPTURER::d3d8-capture.cpp!::hook_d3d8_reset_when_jmp_patched]");

	bool hook_complete_1 = present.ready && present.origin_jmp_patched;
	bool res = false;
	...
	hlog("[NCCAPTURER::d3d8-capture.cpp!::hook_d3d8_reset_when_jmp_patched] loop(%d) res(%d)", i, res);
	return true;
}
