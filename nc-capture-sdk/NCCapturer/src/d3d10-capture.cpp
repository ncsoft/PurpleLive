...
static bool create_d3d10_stage_surface(ID3D10Texture2D **tex)
{
	...
	hlog("[NCCAPTURER::d3d10-capture.cpp!::create_d3d10_stage_surface] device(0x%x) CreateTexture2D", data.device);
			     ...
			     HANDLE *handle)
{
	...
	hlog("[NCCAPTURER::d3d10-capture.cpp!::create_d3d10_tex] device(0x%x) CreateTexture2D", data.device);
	hr = data.device->CreateTexture2D(&desc, nullptr, tex);
	if (FAILED(hr)) {
