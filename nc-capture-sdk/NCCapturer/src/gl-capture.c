...
#define DUMMY_WINDOW_CLASS_NAME L"nccapturer_gl_dummy_window"
...
static struct detour_hook swap_buffers;
static struct detour_hook wgl_swap_layer_buffers;
static struct detour_hook wgl_swap_buffers;
static struct detour_hook wgl_delete_context;
...
static BOOL WINAPI hook_swap_buffers(HDC hdc)
{
	...
	BOOL(WINAPI * call)(HDC) = swap_buffers.pOrigin;
	ret = call(hdc);
...
static BOOL WINAPI hook_wgl_swap_buffers(HDC hdc)
{
	...
	BOOL(WINAPI * call)(HDC) = wgl_swap_buffers.pOrigin;
	ret = call(hdc);
...
static BOOL WINAPI hook_wgl_swap_layer_buffers(HDC hdc, UINT planes)
{
	...
	BOOL(WINAPI * call)(HDC, UINT) = wgl_swap_layer_buffers.pOrigin;
	ret = call(hdc, planes);
...
static BOOL WINAPI hook_wgl_delete_context(HGLRC hrc)
{
	...
	BOOL(WINAPI * call)(HGLRC) = wgl_delete_context.pOrigin;
	ret = call(hrc);
...
bool hook_gl(void)
{
	...
	detour_hook_start(&swap_buffers, SwapBuffers, hook_swap_buffers, "SwapBuffers");
	if (wgl_dc_proc) {
		detour_hook_start(&wgl_delete_context, wgl_dc_proc, hook_wgl_delete_context, "wglDeleteContext");
	}
	if (wgl_slb_proc) {
		detour_hook_start(&wgl_swap_layer_buffers, wgl_slb_proc, hook_wgl_swap_layer_buffers, "wglSwapLayerBuffers");
	}
	if (wgl_sb_proc) {
		detour_hook_start(&wgl_swap_buffers, wgl_sb_proc, hook_wgl_swap_buffers, "wglSwapBuffers");
	}
...
void unhook_gl(void)
{
	hlog("[NCCAPTURER::gl-capture.cpp!::unhook_gl]");

	detour_hook_stop(&swap_buffers);
	detour_hook_stop(&wgl_swap_layer_buffers);
	detour_hook_stop(&wgl_swap_buffers);
	detour_hook_stop(&wgl_delete_context);
}

bool hook_gl_reset_when_jmp_patched(void)
{
	hlog("[NCCAPTURER::gl-capture.cpp!::hook_gl_reset_when_jmp_patched]");

	bool hook_complete_1 = detour_hook_is_complete_when_jmp_patched(&swap_buffers);
	bool hook_complete_2 = detour_hook_is_complete_when_jmp_patched(&wgl_swap_layer_buffers);
	bool hook_complete_3 = detour_hook_is_complete_when_jmp_patched(&wgl_swap_buffers);
	bool hook_complete_4 = detour_hook_is_complete_when_jmp_patched(&wgl_delete_context);
	bool res = false;
	...
	{
		if (!hook_complete_1)
			hook_complete_1 = detour_hook_restart_when_jmp_patched(&swap_buffers);
		if (!hook_complete_2)
			hook_complete_2 = detour_hook_restart_when_jmp_patched(&wgl_swap_layer_buffers);
		if (!hook_complete_3)
			hook_complete_3 = detour_hook_restart_when_jmp_patched(&wgl_swap_buffers);
		if (!hook_complete_4)
			hook_complete_4 = detour_hook_restart_when_jmp_patched(&wgl_delete_context);
		
		if (hook_complete_1 && hook_complete_2 && hook_complete_3 && hook_complete_4) {
			res = true;
			break;
		}
	}
	hlog("[NCCAPTURER::gl-capture.cpp!::hook_gl_reset_when_jmp_patched] loop(%d) res(%d)", i, res);
	return true;
}
