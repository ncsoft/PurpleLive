...
struct encoder_frame {
	...
	uint8_t *data[AMF_MAX_AV_PLANES];

	/** size of each plane */
	uint32_t linesize[AMF_MAX_AV_PLANES];
...
#pragma region Control
			...
			bool Encode(struct encoder_frame* f, struct encoder_packet* p, bool* b, bool is_key_frame);
...
#pragma endregion Control
			...
			virtual std::string HandleTypeOverride(amf::AMFSurfacePtr& d, uint64_t index, bool is_key_frame)    = 0;

			bool EncodeAllocate(OUT amf::AMFSurfacePtr& surface);
			bool EncodeStore(OUT amf::AMFSurfacePtr& surface, IN struct encoder_frame* frame, IN bool is_key_frame);
