...
bool Plugin::AMD::Encoder::Encode(struct encoder_frame* frame, struct encoder_packet* packet, bool* received_packet, bool is_key_frame)
{
	AMFTRACECALL;
	...
	if (!EncodeAllocate(surface))
		return false;
	if (!EncodeStore(surface, frame, is_key_frame))
		return false;
...
bool Plugin::AMD::Encoder::EncodeStore(OUT amf::AMFSurfacePtr& surface, IN struct encoder_frame* frame, IN bool is_key_frame)
{
	AMFTRACECALL;
	...
	/// Type override
	std::string printableType = HandleTypeOverride(surface, frame->pts, is_key_frame);

	if(is_key_frame) {
		// this is not work - insert sps, pps
	...
	if (m_Debug) {
		...
		if (m_Codec == Codec::AVC || m_Codec == Codec::SVC) {
			...
			case AMF_VIDEO_ENCODER_OUTPUT_DATA_TYPE_IDR:
				ELOGD << format_string("[Plugin::AMD::Encoder::EncodeLoad] FrameType(%d) IDR", (int)type);
				printableType = "IDR";
				break;
