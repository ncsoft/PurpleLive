...
class CustomWriter : public amf::AMFTraceWriter {
	...
	virtual void __cdecl Write(const wchar_t* scope, const wchar_t* message) override
	{
		...
		//ELOGD << "[AMF Runtime] " << std::wstring(realmsg);
	}
...
const wchar_t* clc_loggername = L"CLCWriter";
const wchar_t* obs_loggername = L"OBSWriter";
...
/// Register Trace Writer and disable Debug Tracing.
	...
	if (m_AMFVersion_Runtime >= AMF_MAKE_FULL_VERSION(1, 4, 4, 0)) {
		...
		m_AMFTrace->RegisterWriter(clc_loggername, m_TraceWriter, true);
		bool isObsWriterEnabled = m_AMFTrace->WriterEnabled(obs_loggername);
		if(isObsWriterEnabled) {
			m_AMFTrace->EnableWriter(obs_loggername, false);
		}
		isObsWriterDisabled = !isObsWriterEnabled;
#ifndef _WIN64
	}
#endif
...
Plugin::AMD::AMF::~AMF()
{
	...
	if (m_AMFModule) {
		if (m_TraceWriter) {
			if (m_AMFTrace) {
				m_AMFTrace->UnregisterWriter(clc_loggername);
				if(isObsWriterDisabled) {
					m_AMFTrace->EnableWriter(obs_loggername, true);
				}
			}
