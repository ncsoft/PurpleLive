...
static bool nvenc_init(void *nvenc_data, void *encoder_config)
{
	...
	/*
	NV_ENC_PRESET_LOW_LATENCY_HQ_GUID: This preset is desgined to give the highest quality but at the least speed among all the low latency presets.
	...
	NV_ENC_PRESET_LOW_LATENCY_HP_GUID: This is the preset designed to give the maximum speed but the quality is expected to be the lowest among the three low latency presets. 
	*/

	GUID encoder_preset = NV_ENC_PRESET_LOW_LATENCY_DEFAULT_GUID;
	switch (config->quality_preset)
	{
		case NvencQualityPreset::Speed :
		...
		case NvencQualityPreset::Balanced :
			...
			break;
		case NvencQualityPreset::Quality :
			encoder_preset = NV_ENC_PRESET_LOW_LATENCY_HQ_GUID;
			break;
	}

	enc->nvenc->CreateDefaultEncoderParams(&initializeParams, codecId, encoder_preset);
	...
	NV_ENC_PARAMS_RC_MODE rc_mode = NV_ENC_PARAMS_RC_VBR;
	switch (config->rate_control)
	{
		case NvencRateControl::CONSTQP :
			rc_mode = NV_ENC_PARAMS_RC_CONSTQP;
			break;
		case NvencRateControl::VBR :
			rc_mode = NV_ENC_PARAMS_RC_VBR;
			break;
		case NvencRateControl::CBR :
			rc_mode = NV_ENC_PARAMS_RC_CBR;
			break;
		case NvencRateControl::CBR_LOWDELAY_HQ :
			rc_mode = NV_ENC_PARAMS_RC_CBR_LOWDELAY_HQ;
			break;
		case NvencRateControl::CBR_HQ :
			rc_mode = NV_ENC_PARAMS_RC_CBR_HQ;
			break;
		case NvencRateControl::VBR_HQ :
			rc_mode = NV_ENC_PARAMS_RC_VBR_HQ;
			break;
	}

	initializeParams.encodeConfig->rcParams.rateControlMode = rc_mode;
