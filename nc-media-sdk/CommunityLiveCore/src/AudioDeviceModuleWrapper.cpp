...
void AudioDeviceModuleWrapper::onIncomingData(uint8_t* data, size_t samples_per_channel)
{
	...
	if (pendingLength)
	{
		...
		//Add sent
		audioTransport->RecordedDataIsAvailable(pending, chunk, sample_size*channels, channels, (uint32_t)sample_rate, 0, 0, 0, 0, level);
	...
	{
		//Send them
		audioTransport->RecordedDataIsAvailable(data + i*sample_size*channels, chunk, sample_size*channels, channels, (uint32_t)sample_rate, 0, 0, 0, 0, level);
		//Inc sent
		i += chunk;
	}
