...
std::string ELPP_LOG_FILE_PATH = "";
bool ELPP_ENABLE_LOG_ERROR = false;
bool ELPP_ENABLE_LOG_INFO = false;
bool ELPP_ENABLE_LOG_DEBUG = false;

void elpp_set_path(const char* path)
{
	CreateDirectoryA(path, NULL);
	ELPP_LOG_FILE_PATH = get_datetime_path(path, ELPP_DEFAULT_LOG_FILE, "log", true);;
	ELPP->initialize(el::LogBuilderPtr(new el::base::DefaultLogBuilder()));
}

void elpp_set_enable_log(bool error, bool info, bool debug)
{
	ELPP_ENABLE_LOG_ERROR = error;
	ELPP_ENABLE_LOG_INFO = info;
	ELPP_ENABLE_LOG_DEBUG = debug;
}
...
RegisteredLoggers::RegisteredLoggers(const LogBuilderPtr& defaultLogBuilder) :
  ...
  el::base::consts::kDefaultLogFile = ELPP_LOG_FILE_PATH.c_str();
...
#else
Storage::Storage() {};
void Storage::initialize(const LogBuilderPtr& defaultLogBuilder)
{
...
void Writer::processDispatch() {
	...
	if ((!ELPP_ENABLE_LOG_DEBUG && m_level == Level::Debug) ||
		(!ELPP_ENABLE_LOG_INFO && m_level == Level::Info) ||
		(!ELPP_ENABLE_LOG_ERROR && m_level == Level::Error))
	{
		if (m_logger != nullptr) {
...
std::string format_string(const char *fmt, ...)
{
	...
	while (1) {
		buffer.resize(size);
		va_start(ap, fmt);
		int n = vsnprintf((char *)buffer.data(), size, fmt, ap);
		va_end(ap);
		if (n > -1 && n < size) {
			buffer.resize(n);
			return buffer;
...
const char *wstr_to_str(const wchar_t *wstr)
{
...
std::string get_datetime_path(const char* path, const char* name, const char* extension, bool noSpace)
{
	time_t now = time(0);
	struct tm cur_time;
	localtime_s(&cur_time, &now);
	return format_string("%s\\%s_%d-%02d-%02d%c%02d-%02d-%02d.%s",
		path, name,
...
void elog(el::Level level, const char *format, va_list args)
{
...
void elog(el::Level level, const char *format, ...)
{
	va_list args;
	va_start(args, format);
	elog(level, format, args);
	va_end(args);
}

void elogd(const char *format, ...)
{
	va_list args;
	va_start(args, format);
	elog(el::Level::Debug, format, args);
	va_end(args);
}

void elogi(const char *format, ...)
{
	va_list args;
	va_start(args, format);
	elog(el::Level::Info, format, args);
	va_end(args);
}

void eloge(const char *format, ...)
{
	va_list args;
	va_start(args, format);
	elog(el::Level::Error, format, args);
	va_end(args);
}