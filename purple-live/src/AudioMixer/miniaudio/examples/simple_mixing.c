...
int simple_mixing(int input_count, char** input_files)
{
    ma_result result;
    ...
    for (iDecoder = 0; iDecoder < g_decoderCount; ++iDecoder) {
        result = ma_decoder_init_file(input_files[iDecoder], &decoderConfig, &g_pDecoders[iDecoder]);
